...
<head>
    ...
    <title>Report Peminjaman - {{ $datapinjam[0]->pj_pinjam }}</title>
...
<body style="padding-top: 25px; padding-left: 0px;">
    @php
        $cabang = DB::table('tbl_cabang')
            ...
            ->where('kd_cabang', $datapinjam[0]->kd_cabang)
            ->get();
        $staffantarcabang = DB::table('tbl_staff')
            ->select('nama_staff')
            ->where('nip', $datapinjam[0]->pj_pinjam_cabang)
            ->first();
    @endphp
    <div class="header">
        ...
        <h5 style="padding-top: 20px; margin: 20px; left: 100px; padding-left: 155px;text-decoration: underline;">FORM
            PEMAKAIAN BARANG INVENTARIS</h5>
        ...
        <div class="absolute">
            <img style="padding-top: 1px; left: 10px;" src="data:image/png;base64, {!! base64_encode(
                QrCode::style('round')->eye('circle')->format('svg')->size(101)->errorCorrection('H')->generate($datapinjam[0]->tiket_peminjaman),
            ) !!}">
        </div>
    </div>
    <div class="body">
        ...
        <table
            ...
            <tr>
                ...
                <td style="width: 5px;">:</td>
                @php
                    $namapj = DB::table('tbl_staff')
                        ->where('nip', $datapinjam[0]->pj_pinjam)
                        ->first();
                @endphp
                ...
                <td>
                    @php
                        $cabang1 = DB::table('tbl_cabang')
                            ->select('nama_cabang')
                            ->where('kd_cabang', $datapinjam[0]->tujuan_cabang)
                            ->get();
                    @endphp
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @if ($item->status_sub_peminjaman == 0)
                                ...
                                Barang Keluar
                            @else
                                <img style="padding-top: 0px; margin: 2px; left: 2px; ;" src="ceklist.png"
                                    width="15">
                            @endif
                        </td>
                    </tr>
        ...
        <div class="footer">
            <table
                ...
                <tr>
                    ...
                    <td colspan="3" class="text-right"><strong>{{ $cabang[0]->city }} ,
