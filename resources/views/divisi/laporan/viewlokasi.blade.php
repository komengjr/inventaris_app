<div class="modal-content">
        ...
        <form method="post" id="form-report-pilihan-ruangan">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" name="panjang_" id="panjang_">
                </div>
                <div class="col-md-6">
                    ...
                    <select class="form-control single-select" name="kd_lokasi" id="ruangan">
                        ...
                        @foreach ($data as $data)
                            <option value="{{ $data->id_nomor_ruangan_cbaang }}"
                                data-id="{{ $data->id_nomor_ruangan_cbaang }}">{{ $data->nomor_ruangan }} -
    ...
    <div class="modal-footer">
        <button type="button" class="btn-dark" id="button-cetak-barcode-ruangan"
            data-url="{{ url('menu/postmasterlaporan/lokasi-barcode-barang-cabang-ruangan', []) }}"><i
                class="fa fa-download"></i> Print Barcode</button>
        <button type="button" class="btn-primary" id="button-cetak-excel-ruangan"><i class="fa fa-download"></i> Export
            Excel</button>
        <button type="button" class="btn-success" id="button-print-laporan-ruangan-pdf"
            data-url="{{ url('menu/postmasterlaporan/lokasi-barang-cabang-ruangan', []) }}"><i class="fa fa-print"></i>
            Export PDF</button>
    </div>
</div>
...
<script>
    $(document).on("click", "#button-cetak-excel-ruangan", function(e) {
        ...
        $.ajax({
            ...
            .done(function(data) {
                ...
                $("#hasil-report-ruangan").html(
                    '<span class="badge badge-success shadow-success m-1">Downloaded</span>');
            })
            .fail(function() {
    ...
    $(document).on("click", "#button-cetak-barcode-ruangan", function(e) {
        ...
        $.ajax({
            ...
            .done(function(data) {
                if (data == 'null') {
                    $("#hasil-report-ruangan").html('<span class="badge badge-danger shadow-danger m-1">Mohon Isi Semua Element</span>');
                } else {
                    $("#hasil-report-ruangan").html(
